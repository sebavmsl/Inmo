...
export default async function PlanillaPage() {
  ...
  // superadmin no tiene empresa_id propio: `perfil.empresaId` ya refleja
  // la empresa elegida en el selector global (Sidebar, siempre visible —
  // ver lib/auth/session.ts), o null = huérfanos por defecto. Los demás
  // roles ya quedan acotados a la suya por RLS, así que pasan `undefined`
  // (sin filtro adicional acá).
  const empresaFiltro: number | null | undefined = esSuperadmin ? perfil.empresaId : undefined;
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-brand-500">
        ...
        {esSuperadmin && empresaFiltro === null && " · sin empresa asignada"}
      </p>
