...
begin
  ...
  values (p_empresa_id, p_codigo_contrato, p_tipo, current_date, true)
  ...
  where c.estado = 'Activo'
    ...
    and (
      (r.tipo = 'vencimiento' and c.fin_contrato is not null
        and (c.fin_contrato - current_date) = r.dias_antes)
      or
      (r.tipo = 'actualizacion' and c.prox_actualizacion is not null
        and (c.prox_actualizacion - current_date) = r.dias_antes)
    )
    -- No repetir si ya se reservó/envió hoy para este contrato+tipo
    and not exists (
      select 1 from whatsapp_recordatorios_log l
      where l.codigo_contrato = c.codigo and l.tipo = r.tipo and l.fecha_envio = current_date
    );
end;
$$;
