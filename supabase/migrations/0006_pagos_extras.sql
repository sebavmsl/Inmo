...
update contratos set
  ...
  garantia_pagada_base = coalesce(garantia, 0),
...
begin
  ...
  perform pg_advisory_xact_lock(hashtext(p_codigo_contrato));

  -- 2. Leer el contrato FRESCO, ya adentro del candado.
  select codigo, alias_propiedad, saldo_actual, honorarios_pct, mes_contrato
  into v_contrato
  from contratos
  ...
  v_gasto_admin := round(v_monto_para_comision * coalesce(v_contrato.honorarios_pct, 0) / 100.0, 2);
