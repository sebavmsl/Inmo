...
returns table (
  codigo text, empresa_id int, alias_propiedad text, monto_inicial numeric,
  fecha_inicio date, indice text, frecuencia_meses int
)
language sql
security definer
stable
as $$
  select
    c.codigo, c.empresa_id, c.alias_propiedad, c.monto_inicial, c.fecha_inicio,
    c.indice,
    case c.frecuencia_actualizacion
      when 'mensual' then 1 when 'bimestral' then 2 when 'trimestral' then 3
      when 'cuatrimestral' then 4 when 'semestral' then 6 when 'anual' then 12
      else 6
    end as frecuencia_meses
  from contratos c
  ...
  where c.estado = 'Activo'
    ...
    and date_trunc('month', c.prox_actualizacion) = date_trunc('month', current_date)
    ...
    and (
      ...
      or date_trunc('month', c.alquiler_calculado_fecha) != date_trunc('month', current_date)
    );
$$;
