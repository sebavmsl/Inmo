    ...
    <div className="space-y-5 rounded-lg border border-brand-100 bg-white p-5">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        <label className="block text-sm">
          ...
          <select value={datos.frecuenciaMeses} onChange={(e) => set("frecuenciaMeses", Number(e.target.value))} className="w-full rounded border border-brand-100 px-2 py-1.5">
            {FRECUENCIAS.map((f) => <option key={f.meses} value={f.meses}>{f.label}</option>)}
          </select>
        </label>
        <label className="block text-sm">
      ...
      <label className="block max-w-xs text-sm">
        <span className="mb-1 block text-brand-700">% de comisión mensual (honorarios)</span>
