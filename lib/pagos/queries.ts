...
export async function getContratoParaPago(codigo: string): Promise<ContratoParaPago | null> {
  ...
  if (["ICL", "IPC", "UVA"].includes(contrato.indice)) {
    const frecuenciaMeses = frecuenciaAMeses(contrato.frecuencia_actualizacion);
    try {
  ...
  return {
    ...
    honorariosPct: contrato.honorarios_pct ?? 7,
...
function frecuenciaAMeses(frecuencia: string): number {
