...
export interface ContratosRow {
  // OJO: `contratos` NO tiene columna `id` en la base real — su clave es
  // `codigo` (integer). A diferencia de las demás tablas de negocio
  // (inquilinos, propiedades, pagos_historial), acá no hay un id serial.
  /**
   * ⚠️ En la base real, `contratos.codigo` es `integer` (confirmado por
   * volcado de esquema de producción), no texto. Se mantiene tipado como
   * `string` acá a propósito — PostgREST/Postgres castean automáticamente
   * un string numérico ("1024") contra una columna integer tanto al leer
   * (JSON number → puede tratarse como string sin problema en JS) como al
   * escribir vía json_populate_record, así que es seguro en runtime. Se
   * eligió no tocar el tipo TS para no arrastrar un cambio en cascada por
   * todo el código (firmas de función, comparaciones) sin poder compilar
   * y verificar en este entorno. Si en algún momento se puede validar con
   * un build completo, lo correcto sería `number`.
   */
  codigo: string;
  empresa_id: number;
  ...
  /**
   * % de comisión mensual — alimenta pagos_historial.monto_gasto_admin.
   * OJO: el nombre real de la columna es `honorarios` (confirmado contra
   * app.py, ej. línea 4345: `monto_abonado * c_datos.get('honorarios') / 100.0`).
   * La columna `honorarios_pct` que se había asumido acá no existe.
   */
  honorarios: number;
  /**
   * Frecuencia de actualización del alquiler, YA expresada en cantidad de
   * meses (1, 2, 3, 4, 6, 12, 24...) — confirmado contra app.py, línea
   * ~2559 (`opciones_meses`), donde el fallback a texto solo es defensivo
   * para datos viejos; el tipo real de la columna en Postgres es `integer`.
   * Reemplaza a la columna inventada `frecuencia_actualizacion` (que
   * asumía un enum de texto y no existe en la base real).
   */
  act_contrato: number | null;
